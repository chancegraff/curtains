...
function convertMarkdownToAST(mdast: ReturnType<typeof parseMarkdown>): ASTNode[] {
  ...
  const nodes: ASTNode[] = mdast.children.flatMap((child: MarkdownNode) =>
    convertMarkdownNode(child)
  );
  return nodes;
}
    ...
    case 'heading': {
      const headingNode: ASTNode = HeadingNodeSchema.parse({
        ...
        children: node.children
          ? node.children.flatMap((child: MarkdownNode) => convertMarkdownNode(child))
          : [],
      });
      return [headingNode];
    }
 ...
 */
export function buildAST(containerResult: ContainerParseResult): z.infer<typeof CurtainsASTSchema> {
