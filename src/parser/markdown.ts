 ...
 */
function sanitizeImgAttributes(imgTagMatch: string): {
  src: string;
  alt: string;
  classes?: string[];
} {
  ...
  switch (node.type) {
    ...
    case 'emphasis':
      ...
      if (
        ...
        result.children =
          node.children?.map((child: MdastNode) => {
