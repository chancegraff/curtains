...
export function normalizeCommand(command: string): CLICommand {
  ...
  return cmd === 'build' || cmd === 'serve' || cmd === 'watch' || 
         cmd === 'export' || cmd === 'validate' ? cmd : 'build'
}
...
export function normalizePluginsList(pluginsValue: string): string[] {
  ...
  return pluginsValue.split(',').map(p => p.trim()).filter(Boolean)
}
...
export function isArgumentError(errorMessage: string): boolean {
  return errorMessage.includes('Command') || 
...
export const validateCommand = (command: string): CLICommand => {
  return command === 'build' || command === 'serve' || command === 'watch' ||
    command === 'export' || command === 'validate'
    ? command
    : 'build'
}
...
export const handleBuildCommand = async (context: CLIContext, fileOps: FileOperations): Promise<CLIResult> => {
...
export const handleServeCommand = async (context: CLIContext, _fileOps: FileOperations): Promise<CLIResult> => {
...
export const handleWatchCommand = async (context: CLIContext, fileOps: FileOperations): Promise<CLIResult> => {
...
export const handleExportCommand = async (context: CLIContext, _fileOps: FileOperations): Promise<CLIResult> => {
...
export const handleValidateCommand = async (context: CLIContext, fileOps: FileOperations): Promise<CLIResult> => {
  ...
  try {
    ...
    if (!context.pipeline || typeof context.pipeline !== 'object' || !('registry' in context.pipeline)) {
      throw new Error('Pipeline not properly initialized')
    }
...
// Command dispatcher
export const executeCommand = async (context: CLIContext, fileOps: FileOperations): Promise<CLIResult> => {
  try {
...
export const runCLIIfDirect = async (importMetaUrl: string, processArgv: string[]): Promise<void> => {
